      ...
      <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
        <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
            ...
            <div className='relative flex flex-col gap-3'>
              ...
              <p>{timeFormat(show.movie.runtime)} . {show.movie.genres.map((item)=>item.name).join("| ")} . {show.movie.release_date.split("-")[0]}</p>
        ...
        <div className='flex flex-wrap max-sm:justify-center gap-8'>
              {dummyShowsData.slice(0,6).map((data)=>(
                  <Moviecard movie={data} key={data._id}/>
              ))}
        </div>
        <div className='flex justify-center mt-20'>
           <button onClick={()=>{navigate("/movies");scrollTo(0,0)}} className='px-10 py-3 text-sm bg-primary hover:primary-dull transition rounded-md font-medium cursor-pointer'>Show More...</button>
        </div>
      </div>
  ):
  (
